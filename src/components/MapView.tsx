...
const EventMarker = memo(({
  ...
  return (
    ...
    >
      <div
        id={`map-pin-${event.id}`}
        className="event-marker-el"
        style={{ cursor: "pointer", transform: `scale(${scale})`, position: "relative" }}
      >
        {isLive && (
          <div
            style={{
...
export default function MapView({
  ...
  const zoom = Math.round(viewState.zoom ?? 12);
  ...
  // Sync bounds ONLY when map stops moving or on idle.
  // This prevents continuous updates (and heavy React re-renders) on every single frame during transition.
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      <Map
        ...
        onLoad={handleIdle}
        onIdle={handleIdle}
        onMoveEnd={handleIdle}
        onZoomEnd={handleIdle}
        mapStyle={mapStyle}
        ...
        {clusters.map((cluster: any) => {
          ...
          if (isCluster) {
            ...
            try {
              ...
              const urgencyOf = (l: any) => {
                ...
                if (s.toDateString() === todayStr) return 1;
                if (s.toDateString() === tomorrowStr) return 2;
          ...
          return (
            <EventMarker
              ...
              zoom={zoom}
              eventCountAtLocation={cluster.properties.eventCountAtLocation}
              onClick={(e) => {
                e.originalEvent.stopPropagation();
                onEventSelect(event);
              }}
      ...
      <style jsx global>{`
        ...
        .cluster-marker-el, .event-marker-el {
          transition: transform 0.15s cubic-bezier(0.16, 1, 0.3, 1);
        }
        .cluster-marker-el:hover, .event-marker-el:hover {
