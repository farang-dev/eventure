...
export async function generateMetadata(props: { params: Promise<{ city: string }> }) {
  ...
  const url = `https://www.eventurer.online/events/${city}`;
  return {
...
export default async function CityEventsPage(props: { params: Promise<{ city: string }> }) {
  ...
  const cityUrl = `https://www.eventurer.online/events/${city}`;
  ...
  const cityLd = [
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Events", item: "https://www.eventurer.online/events" },
        { "@type": "ListItem", position: 3, name: info.name, item: cityUrl },
      ],
    },
    {
      "@context": "https://schema.org",
