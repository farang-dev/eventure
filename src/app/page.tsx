...
export default function HomePage() {
  ...
  // Sync theme with map style (Client-side only)
  useEffect(() => {
    if (!isMounted) return;
  ...
  const events = useMemo(() => {
    ...
    return eventsData
      ...
      .filter(e => {
        ...
        // Strict deduplication: ignore slight coordinate differences
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((e) => {
      ...
      if (searchQuery) {
        ...
        // If it doesn't match text, it might still match the city if we are just searching for a city
        // but typically we want the search to filter the list
  ...
  // Filter list by what is currently visible on the map (Bounds filtering)
  const visibleEvents = useMemo(() => {
    return filteredEvents.filter(e => {
      if (!mapBounds) return true; // Show all if bounds not yet set
      const [west, south, east, north] = mapBounds;
          ...
          <>
            ...
            <div className="top-header">
              ...
              <div className="top-header-row">
                ...
                {/* Search Bar - Horizontal on Desktop, Moved to Row 2 on Mobile via CSS */}
                <div
