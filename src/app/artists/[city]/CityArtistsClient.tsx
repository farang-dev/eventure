...
import { Search, ArrowLeft, Users, X, Building2, Map as MapIcon, ChevronRight, Share2, Check } from "lucide-react";
...
export default function CityArtistsClient({ initialArtist }: Props) {
  ...
  // ── Share state and handlers ──────────────────────────────────────────────────
  const [copied, setCopied] = useState(false);

  const getArtistShareUrl = () => {
    if (!selectedArtist) return "";
    const slug = encodeURIComponent(selectedArtist.toLowerCase().replace(/\s+/g, "-"));
    return `https://www.eventurer.online/artists/${cityId}/${slug}`;
  };

  const getArtistShareText = () => {
    if (!selectedArtist) return "";
    return `Check out ${selectedArtist} on Eventure — live DJ sets & upcoming gigs in ${cityMeta?.name || cityId}`;
  };

  const handleNativeShare = async () => {
    const url = getArtistShareUrl();
    const title = getArtistShareText();
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText(getArtistShareUrl());
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch {}
  };
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20, width: isMobile ? "100%" : undefined }}>
              {selectedArtist ? (
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
                  ...
                  <a
                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(getArtistShareText())}&url=${encodeURIComponent(getArtistShareUrl())}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: "inline-flex",
                  ...
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(getArtistShareText() + " " + getArtistShareUrl())}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: "inline-flex",
                  ...
                  >
                    {copied ? <Check size={14} /> : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>}
